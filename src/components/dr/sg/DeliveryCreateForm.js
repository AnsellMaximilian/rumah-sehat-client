...
import { getSubtotal } from "../../../helpers/drSg";
  ...
  useEffect(() => {
    (async () => {
      ...
      if (editId && items.length > 0) {
        const delivery = (await http.get(`/dr/sg/deliveries/${editId}`)).data
          .data;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!editId) {
        ...
        await http.patch(`/dr/sg/deliveries/${editId}`, body);
              ...
              <TableBody>
                {deliveryDetails.map((detail) => (
                  ...
                  >
                    ...
                    <TableCell align="right">
                      <NumericFormatSGD
                        value={(detail.priceSGD * detail.qty).toFixed(0)}
                      />
                    </TableCell>
                  </TableRow>
                ))}
                <TableRow>
                  ...
                  <TableCell align="right">
                    <NumericFormatSGD
                      value={parseInt(getSubtotal(deliveryDetails, "priceSGD"))}
                    />
                  </TableCell>
                </TableRow>
                <TableRow>
                  ...
                  <TableCell align="right">
                    <NumericFormatRp
                      value={
                        parseInt(getSubtotal(deliveryDetails, "priceSGD")) *
                          exchangeRate +
                        (useIndividualDeliveryCost
                          ? parseInt(
                              getSubtotal(deliveryDetails, "deliveryCost")
