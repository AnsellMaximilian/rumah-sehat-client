...
export default function DrIdItemShow() {
  const { id } = useParams();
  ...
  const handleAdjust = async () => {
    try {
      ...
      const adjust = (await http.post(`/dr/id/items/${id}/adjust-stock`, body))
        .data.data;
      refreshMetaData();
      setAdjustAmount(0);
      setAdjustDate("");
  ...
  const handleLend = async () => {
    try {
      ...
      const lend = (await http.post(`/dr/id/loans`, body)).data.data;
      refreshMetaData();
      setLendQty(0);
      setLendDate("");
      setLendNotes("");
  ...
  const handleDeleteLoan = (id) => {
    (async () => {
      try {
        await http.delete(`/dr/id/loans/${id}`);
        refreshMetaData();
  ...
  const handleReturn = async () => {
    try {
      ...
      await http.patch(`/dr/id/loans/${selectedReturnLoan.id}/return`, {
            ...
            <Stack spacing={2}>
              ...
              <SmartTable
                rows={[...loans, ...bundleLoans].map((loan) => ({
                  ...
                  loanItem: loan,
                  bundleItem:
                    loan.DrIdItem?.id !== Number(id) ? loan.DrIdItem : null,
                }))}
                columns={lendColumns}
              />
            </Stack>
          ...
          <Stack padding={4} spacing={2} minWidth={500}>
            <Typography fontWeight="bold" fontSize={24}>
              {selectedReturnLoan.lendType === "lend"
                ? `Take ${selectedReturnLoan.qty} ${selectedReturnLoan.DrIdItem.name} back from ${selectedReturnLoan.customerFullName}?`
                : `Return ${selectedReturnLoan.qty} ${selectedReturnLoan.DrIdItem.name} to ${selectedReturnLoan.customerFullName}?`}
            </Typography>
            <Stack spacing={2} direction="row">
              <TextField
                label="Return Date"
